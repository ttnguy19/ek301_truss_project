...
% Calculate the force components for each member
    ...
    % Calculate the coefficient and adding them to matrix Ax and Ay
        Ax(indices(1), i) = (X(indices(2)) - X(indices(1))) /  member_length;
        Ax(indices(2), i) = (X(indices(1)) - X(indices(2))) /  member_length;
        Ay(indices(1), i) = (Y(indices(2)) - Y(indices(1))) /  member_length;
        Ay(indices(2), i) = (Y(indices(1)) - Y(indices(2))) /  member_length;
